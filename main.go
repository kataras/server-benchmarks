...
var (
	waitServerDur = flag.Duration("wait-server", 8*time.Second, "wait time for server readiness")
	...
	spreadsheetID = flag.String("g-spreadsheet", "", "(not available yet) Google Spreadsheet ID to send results")
	...
	googleSecretFile = flag.String("g-secret", "client_secret.json", "(not available yet) if g-spreadsheet is set, service account credentials file should be provided")
)
